...
export async function GET(req: Request) {
    try {
        ...
        if (!rl.success) return rateLimitResponse(rl.reset, { req, path: "/api/team-storage", userId: session?.user?.id, userName: session?.user?.name, userEmail: session?.user?.email });
